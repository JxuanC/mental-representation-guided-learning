...
def GW_distance(X, Y, p, q, lamda=0.5, iteration=5, OT_iteration=20, indices = None):
	'''
	...
	Cst = Cst * indices if indices is not None else Cst
	temp = torch.bmm(torch.transpose(Cst,1,2), T)
	distance = batch_trace(temp, m, bs)
	return distance, T, Cst
...
def GW_torch_batch(Cs, Ct, bs, n, m, p, q, beta=0.5, iteration=5, OT_iteration=20):
	...
	return gamma, Cgamma  # return T, L
...
def GW_distance_uniform(X, Y, lamda=1e-1, iteration=5, OT_iteration=20, indices = None):
	m = X.size(2)
	n = Y.size(2)
	bs = X.size(0)
	p = (torch.ones(bs, m, 1)/m).cuda()
	q = (torch.ones(bs, n, 1)/n).cuda()
	return GW_distance(X, Y, p, q, lamda=lamda, iteration=iteration, OT_iteration=OT_iteration, indices=indices)
